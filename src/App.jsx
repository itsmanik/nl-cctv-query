    ...
    <div className="flex h-screen bg-gray-900 text-white">
      ...
      <div className={`flex-1 flex flex-col transition-all duration-300 ${isSidebarOpen ? "ml-64" : "ml-0"}`}>
        <header className="p-4 bg-gray-800 shadow-md flex items-center justify-between">
          ...
          <h1 className="text-2xl font-bold text-blue-400">AI to CCTV Query</h1>
          <Link to="/login" className="text-blue-400">Logout</Link>
        </header>
        <div className="flex-1 p-4 overflow-y-auto space-y-4">
          {messages.map((message, index) => (
            <div key={index} className={`flex ${message.sender === "user" ? "justify-end" : "justify-start"}`}>
              <div className={`max-w-[70%] p-3 rounded-lg ${message.sender === "user" ? "bg-blue-500" : "bg-gray-700"}`}>{message.text}</div>
            </div>
          ))}
        </div>
        <div className="p-4 bg-gray-800 border-t border-gray-700">
          <div className="flex gap-4 items-center">
            <input type="text" value={inputText} onChange={(e) => setInputText(e.target.value)} placeholder="Ask about CCTV footage..." className="flex-1 p-2 rounded-lg bg-gray-700 text-white" />
            <button onClick={handleSendMessage} className="px-4 py-2 bg-blue-500 text-white rounded-lg">Send</button>
          </div>
          <div className="mt-4">
            <label htmlFor="cctv-upload" className="flex items-center p-2 bg-blue-500 rounded-lg cursor-pointer">
              <FaUpload className="mr-2" /> Upload CCTV Footage
            </label>
            <input id="cctv-upload" type="file" onChange={handleFileUpload} className="hidden" />
          </div>
        </div>
      </div>
    </div>
